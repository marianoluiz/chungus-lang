...
class ChungusLexerGUI:
    def __init__(self, root, lexer_callback=None, syntax_callback=None):
        ...
        if os_name == "Darwin":
            ...
            default_font_family = "Segoe UI Variable Text"
            title_font_family = "Segoe UI Variable Display"
            mono_font_family = "Consolas"
        else:
            default_font_family = "Cantarell"
            title_font_family = "Cantarell"
            mono_font_family = "Monospace"
        try:
            self.title_font = font.Font(family=title_font_family, size=24, weight="bold")
        except:
            self.title_font = font.Font(family="Arial", size=24, weight="bold")
        try:
            self.label_font = font.Font(family=default_font_family, size=10)
        except:
            self.label_font = font.Font(family="Arial", size=10)
        try:
            self.label_font_bold = font.Font(family=default_font_family, size=11, weight="bold")
        except:
            self.label_font_bold = font.Font(family="Arial", size=11, weight="bold")
        try:
            self.code_font = font.Font(family=mono_font_family, size=11)
        except:
            self.code_font = font.Font(family="Courier New", size=11)
        try:
            self.token_font = font.Font(family=mono_font_family, size=10)
        except:
            self.token_font = font.Font(family="Courier New", size=10)
        try:
            self.token_font_bold = font.Font(family=mono_font_family, size=10, weight="bold")
        except:
            self.token_font_bold = font.Font(family="Courier New", size=10, weight="bold")
        try:
            self.status_font = font.Font(family=default_font_family, size=9)
        except:
    ...
    def build_widgets(self):
        ...
        self.paned_window = tk.PanedWindow(self.main_frame, orient=tk.HORIZONTAL,
                                           relief="flat", borderwidth=0, sashwidth=8, sashrelief="flat", showhandle=False)
        ...
        self.run_lexer_button = tk.Button(self.button_frame, text="Run Lexer", command=self.run_lexer_only,
                                          font=self.label_font_bold, relief="flat", borderwidth=0, padx=20, pady=10)
        self.run_lexer_button.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 8))

        self.run_syntax_button = tk.Button(self.button_frame, text="Run Syntax", command=self.run_syntax,
                                           font=self.label_font_bold, relief="flat", borderwidth=0, padx=20, pady=10)
        self.run_syntax_button.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(8, 0))
        ...
        self.error_output = tk.Text(self.error_output_frame, height=6, font=self.token_font,
                                    wrap=tk.WORD, borderwidth=0, relief="flat", highlightthickness=0, padx=12, pady=12)
        self.error_scrollbar = ttk.Scrollbar(self.error_output_frame, orient=tk.VERTICAL,
                                             command=self.error_output.yview, style="Vertical.TScrollbar")
        ...
        self.token_tree = ttk.Treeview(self.tree_container_frame, columns=(
            "Line", "Col", "Lexeme", "Token"), show="headings", style="Treeview")
        ...
        self.tree_scrollbar = ttk.Scrollbar(self.tree_container_frame, orient=tk.VERTICAL,
                                            command=self.token_tree.yview, style="Vertical.TScrollbar")
    ...
    def apply_theme(self):
        ...
        self.style.configure('Title.TLabel', foreground=c["TITLE_COLOR"],
                             background=c["BG_COLOR"], font=self.title_font)
        self.style.configure('Bold.TLabel', foreground=c["FG_COLOR"],
                             background=c["BG_COLOR"], font=self.label_font_bold)
        self.style.configure('TPanedwindow', background=c["BG_COLOR"])

        # Treeview Styles - IMPORTANT to reconfigure these
        self.style.configure("Treeview", background=c["TEXT_AREA_BG"], foreground=c["FG_COLOR"],
                             fieldbackground=c["TEXT_AREA_BG"], font=self.token_font)
        self.style.configure("Treeview.Heading", background=c["TREE_HEADING_BG"],
                             foreground=c["FG_COLOR"], font=self.label_font_bold)
        self.style.map('Treeview', background=[('selected', c["ACCENT_BLUE"])],
                       foreground=[('selected', c["BUTTON_FG"])])

        # Scrollbar Styles - IMPORTANT to reconfigure these
        self.style.configure("Vertical.TScrollbar",
                             troughcolor=c["BG_COLOR"], background=c["STATUS_BAR_BG"], arrowcolor=c["SECONDARY_TEXT"])
        self.style.map("Vertical.TScrollbar", background=[('active', c["BORDER_COLOR"])])
        # --------------------------------

        # Update tk widget colors directly
        try:
            self.title_label.configure(background=c["BG_COLOR"], foreground=c["TITLE_COLOR"])
        except:
            pass
        try:
            self.code_label.configure(background=c["BG_COLOR"], foreground=c["FG_COLOR"])
        except:
            pass
        try:
            self.error_label.configure(background=c["BG_COLOR"], foreground=c["FG_COLOR"])
        except:
            pass
        try:
            self.token_label.configure(background=c["BG_COLOR"], foreground=c["FG_COLOR"])
        except:
            pass
        ...
        for frame in [self.code_input_frame, self.error_output_frame, self.tree_container_frame]:
            try:
                frame.configure(background=c["TEXT_AREA_BG"], highlightbackground=c["BORDER_COLOR"])
            except:
                pass
        ...
        try:
            ...
            self.run_lexer_button.configure(bg=button_bg, fg=button_fg,
                                            activebackground=active_bg,
                                            activeforeground=button_fg)
            self.run_syntax_button.configure(bg=button_bg, fg=button_fg,
                                             activebackground=active_bg,
                                             activeforeground=button_fg)
        except:
            pass
        ...
        try:
            ...
            if current_text.startswith(">>>"):
                self.error_output.insert(tk.END, current_text, "success")
            elif current_text:
                self.error_output.insert(tk.END, current_text, "error")
            self.error_output.config(state=tk.DISABLED)
        except:
            pass
    ...
    def set_theme(self):
        ...
        if theme_name in self.themes:
            ...
            self.run_syntax()  # Re-run syntax to redraw treeview with new theme tags
        else:
    ...
    def on_focus_in(self, event):
        widget = event.widget
        frame = None
        if widget == self.code_input:
            frame = self.code_input_frame
        elif widget == self.error_output:
            frame = self.error_output_frame
        elif widget == self.token_tree:
            frame = self.tree_container_frame
        if frame:
    ...
    def on_focus_out(self, event):
        ...
        if widget == self.code_input:
            ...
            frame = self.tree_container_frame
        if frame:
            frame.config(highlightbackground=self.colors["BORDER_COLOR"], highlightthickness=1)
    ...
    # --- File Operations ---
    def open_file(self):
        filepath = filedialog.askopenfilename(title="Open Chungus File", filetypes=[(
    ...
    def save_file(self):
        filepath = filedialog.asksaveasfilename(title="Save Chungus Code As...", defaultextension=".chg", filetypes=[
                                                ("Chungus Files", "*.chg *.chungus"), ("Text Files", "*.txt"), ("All Files", "*.*")])
        if not filepath:
            return
        try:
    ...
    def run_lexer_only(self):
        """ Callback function for the 'Run Lexer' button.

        Only runs the lexical analyzer on the source code.
        """
        self.error_output.config(state=tk.NORMAL)
        self.error_output.delete("1.0", tk.END)
        ...
        if callable(self.lexer_callback):
            ...
            instr = (
                ...
                "Set gui.lexer_callback = adapter and press Run Lexer."
            )
    ...
    def run_syntax(self):
        """ Callback function for the 'Run Syntax' button.

        Runs both the lexical analyzer and syntax parser on the source code.
        """
        self.error_output.config(state=tk.NORMAL)
        self.error_output.delete("1.0", tk.END)
        for item in self.token_tree.get_children():
        ...
        if callable(self.syntax_callback):
            try:
                ...
                messagebox.showerror("Syntax callback error", str(e))
                return
        else:
            # No backend connected — show instructions
            instr = (
                "No syntax backend connected.\n\n"
                "Provide a callable gui.syntax_callback = lambda src: (tokens, errors)\n"
                "Where tokens is a list of objects/dicts with fields: type, lexeme, line, col.\n\n"
                "Example adapter using top-level Lexer and Parser:\n"
                "    from lexer import Lexer\n"
                ...
                "        return tokens, errors\n\n"
                "Set gui.syntax_callback = adapter and press Run Syntax."
            )
            self.error_output.insert(tk.END, instr, "error")
            self.error_output.config(state=tk.DISABLED)
            return
